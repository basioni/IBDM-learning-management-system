...
if (!class_exists('WPLMS_Assignments')){
    ...
    public static function init(){
        ...
        function get_assignment_submission_count($course_id){
            ...
            $count = $wpdb->get_var("SELECT count(DISTINCT p.meta_key) FROM {$wpdb->postmeta} as p LEFT JOIN {$wpdb->comments} as c ON p.post_id = c.comment_post_ID 
        ...
        function get_assignment_submissions($course_id){
            ...
            if(!empty($assignments)){
                ...
                $count_array = array();
                $assignment_ids = implode(',',$assignment_ids);
                $submissions = $wpdb->get_results("SELECT count(DISTINCT p.meta_key) as count, p.post_id FROM {$wpdb->postmeta} as p
